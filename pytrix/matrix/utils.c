...
Point *_pointMatrixMul(PyObject *a, PyObject *b) {
/*  Multiplies a Matrix by a Point.

    Inputs: a - Either the Point or the matrix to multiply.
            b - Either the Point or the matrix to multiply.
...
*/
    ...
    // There are two cases here.  If the point is the first argument, then we perform left-hand point multiplication
    if (Point_Check(a) && Matrix_Check(b)) {
        inPoint = (Point *)a;
        inMatrix = (Matrix *)b;

        if (inPoint->dimensions != inMatrix->rows) {
            PyErr_SetString(PyExc_ValueError, "Point * Matrix multiplication must have P.dimensions = M.rows");
            return NULL;
        }

        outPoint = _pointNew(inPoint->dimensions);

        for (i = 0; i < inMatrix->columns; i++) {
            val = 0;
            for (j = 0; j < inPoint->dimensions; j++) {
                val += Point_GetValue(inPoint, j) * Matrix_GetValue(inMatrix, j, i);
            }

            Point_SetValue(outPoint, i, val);
        }

        return outPoint;

    // If the point is the second, right-hand point multiplication
    } else if (Matrix_Check(a) && Point_Check(b)) {
        inPoint = (Point *)b;
        inMatrix = (Matrix *)a;

        if (inPoint->dimensions != inMatrix->columns) {
            PyErr_SetString(PyExc_ValueError, "Matrix * Point multiplication must have M.cols = P.dimensions");
            return NULL;
        }
        ...
        for (i = 0; i < inMatrix->rows; i++) {
            val = 0;
            for (j = 0; j < inPoint->dimensions; j++) {
                val += Matrix_GetValue(inMatrix, i, j) * Point_GetValue(inPoint, j);
            }
    ...
    PyErr_SetString(PyExc_TypeError, "Undefined inputs. Point-Matrix multiplication requires a Point & Matrix.");
    return NULL;
}
